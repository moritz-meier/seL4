...
var_size_suffix_map = {
    k: "unsigned" + v
    for (k, v) in {8: "_char", 16: "_short", 32: "", 64: "_longlong"}.items()
}
...
ASSERTS = {"sel4": "assert", "libsel4": "seL4_DebugAssert"}

INLINE = {"sel4": "static inline", "libsel4": "LIBSEL4_INLINE_FUNC"}

TYPES = {
    "sel4": {8: "uint8_t", 16: "uint16_t", 32: "uint32_t", 64: "uint64_t"},
    "libsel4": {
        8: "seL4_Uint8",
        16: "seL4_Uint16",
        32: "seL4_Uint32",
...
def p_block(t):
    """block : BLOCK IDENTIFIER opt_visible_order_spec""" """ LBRACE fields RBRACE"""
...
def p_tagged_union(t):
    """tagged_union : TAGGED_UNION IDENTIFIER IDENTIFIER tag_slices""" """ LBRACE masks tags RBRACE"""
    t[0] = TaggedUnion(
        name=t[2], tagname=t[3], tag_slices=t[4], classes=t[6], tags=t[7]
    )
...
typedef_template = """struct %(name)s {
    %(type)s words[%(multiple)d];
};
typedef struct %(name)s %(name)s_t;"""

generator_template = """%(inline)s %(block)s_t CONST
...
ptr_generator_template = """%(inline)s void
...
reader_template = """%(inline)s %(type)s CONST
...
writer_template = """%(inline)s %(block)s_t CONST
...
union_generator_template = """%(inline)s %(union)s_t CONST
...
ptr_union_generator_template = """%(inline)s void
...
union_reader_template = """%(inline)s %(type)s CONST
...
union_writer_template = """%(inline)s %(union)s_t CONST
...
tag_reader_header_template = """%(inline)s %(type)s CONST
%(union)s_get_%(tagname)s(%(union)s_t %(union)s) {
"""

tag_reader_entry_template = """    if ((%(union)s.words[%(index)d] & 0x%(classmask)x) != 0x%(classmask)x)
...
tag_eq_reader_header_template = """%(inline)s int CONST
%(union)s_%(tagname)s_equals(%(union)s_t %(union)s, %(type)s %(union)s_type_tag) {
"""

tag_eq_reader_entry_template = """    if ((%(union)s_type_tag & 0x%(classmask)x) != 0x%(classmask)x)
        return ((%(union)s.words[%(index)d] >> %(shift)d) & 0x%(mask)x%(suf)s) == %(union)s_type_tag;
"""

tag_eq_reader_final_template = """    return ((%(union)s.words[%(index)d] >> %(shift)d) & 0x%(mask)x%(suf)s) == %(union)s_type_tag;"""

tag_eq_reader_footer_template = """
}"""

ptr_tag_reader_header_template = """%(inline)s %(type)s PURE
%(union)s_ptr_get_%(tagname)s(%(union)s_t *%(union)s_ptr) {
"""

ptr_tag_reader_entry_template = """    if ((%(union)s_ptr->words[%(index)d] & 0x%(classmask)x) != 0x%(classmask)x)
...
tag_writer_template = """%(inline)s %(union)s_t CONST
...
ptr_tag_writer_template = """%(inline)s void
...
lift_def_template = '''definition %(name)s_lift :: "%(name)s_C \\<Rightarrow> %(name)s_CL" where
...
union_get_tag_def_header_template = """definition %(name)s_get_tag :: "%(name)s_C \\<Rightarrow> word%(base)d" where
...
union_get_tag_eq_x_def_header_template = """lemma %(name)s_get_tag_eq_x:
  "(%(name)s_get_tag c = x) = (("""
...
union_lift_def_template = '''definition %(name)s_lift :: "%(name)s_C \\<Rightarrow> %(name)s_CL option" where
  "%(name)s_lift %(name)s \\<equiv>
    (let tag = %(name)s_get_tag %(name)s in
     %(tag_cases)s
     else None)"'''

union_access_def_template = '''definition %(union)s_%(block)s_access ::
...
def ptr_basic_template(name, ptrname, retval, args, post):
    return (
        ...
        + name
        + """_spec:
           defines "ptrval s \\<equiv> cslift s """
        + ptrname
        + """"
           shows "\\<forall>s. \\<Gamma> \\<turnstile> \\<lbrace>s. s \\<Turnstile>\\<^sub>c """
        + ptrname
        + """\\<rbrace>
            """
        + retval
...
def ptr_empty_union_new_template(ptrname):
    return ptr_union_basic_template(
        "new",
        ptrname,
        "",
        "",
...
def emit_named_ptr_proof(fn_name, params, name, type_map, toptps, prf_prefix, substs):
    ...
    if name_C in type_map:
        ...
        substs["access_path"] = (
            "("
            + reduce(lambda x, y: y + " (" + x + ")", ["the (ptrval s)"] + path)
            + ")"
        )
        ...
        if len(path) == 0:
            ...
            emit_named(
                fn_name,
                params,
                make_proof(prf_prefix + "_direct", substs, params.sorry),
            )
        else:
            substs["toptp"] = toptp
            ...
            emit_named(
                fn_name, params, make_proof(prf_prefix + "_path", substs, params.sorry)
            )
...
class TaggedUnion:
            ...
            if self.sliced_tag:
                if len(value) > 1:
                    if len(value) != len(self.tag_slices):
                        raise ValueError(
                            "Tag value for element %s of tagged union"
                    ...
                    for i, tag_slice in enumerate(self.tag_slices):
                        ...
                        if value[i] > 2**size - 1:
                            raise ValueError(
                                "Tag value %s for element %s of tagged union"
    ...
    def resolve(self, params, symtab):
        ...
        if self.sliced_tag and self.classes:
            raise ValueError(
                "Tagged union %s has both sliced tags and class masks." % self.name
            )
        ...
        for name, value, ref in self.tags:
            for tag_slice in self.tag_slices:
                ...
                if union_size is None:
                    ...
                    raise ValueError(
                        "Size mismatch for element %s"
                        " of tagged union %s" % (name, self.name)
                    )

                if (
                    _tag_offset
                    != self.tag_offset[tag_slice if self.sliced_tag else _tag_size]
                ):
                    raise ValueError(
                        "Tag offset mismatch for element %s"
                        " of tagged union %s" % (name, self.name)
                    )
                ...
                if self.sliced_tag:
                    if _tag_size != self.tag_size[tag_slice]:
                        raise ValueError(
                            "Tag size mismatch for element %s"
                            " of tagged union %s" % (name, self.name)
                        )
                else:
                ...
                if _tag_high:
                    raise ValueError(
                        "Tag field is high-aligned for element %s"
                        " of tagged union %s" % (name, self.name)
                    )
    ...
    def expanded_tag_val(self, compressed):
        ...
        parts = [
            ((compressed >> position) & ((1 << size) - 1)) << offset
    ...
    def generate_hol_proofs(self, params, type_map):
        ...
        print(
            "lemmas %(name)s_C_words_C_fl_simp[simp] = "
            "%(name)s_C_words_C_fl[simplified]" % {"name": self.name},
            file=output,
        )
        print(file=output)

        # Generate struct field pointer proofs
        substs = {"name": self.name, "words": self.multiple, "base": self.base}

        print(make_proof("words_NULL_proof", substs, params.sorry), file=output)
        print(file=output)

        print(make_proof("words_aligned_proof", substs, params.sorry), file=output)
        print(file=output)

        print(make_proof("words_ptr_safe_proof", substs, params.sorry), file=output)
        print(file=output)

        # Generate struct lemmas
        print(struct_lemmas_template % {"name": self.name}, file=output)
        print(file=output)
        ...
        if not params.skip_modifies:
            ...
            emit_named(
                "%(name)s_ptr_get_%(tagname)s" % substs,
                params,
                make_proof(
                    "const_modifies_proof",
                    {
        ...
        emit_named(
            "%s_get_%s" % (self.name, self.tagname),
            params,
            make_proof("get_tag_spec", substs, params.sorry),
        )

        emit_named(
            "%s_%s_equals" % (self.name, self.tagname),
            params,
        ...
        emit_named_ptr_proof(
            "%s_ptr_get_%s" % (self.name, self.tagname),
            params,
            self.name,
            type_map,
        ...
        for name, value, ref in self.tags:
            ...
            if not params.skip_modifies:
                emit_named(
                    "%s_%s_new" % (self.name, ref.name),
                    params,
                    make_proof(
                        "const_modifies_proof",
                        {
                            "fun_name": "%s_%s_new" % (self.name, ref.name),
                            "args": ", ".join(
                                ["\\<acute>ret__struct_%(name)s_C" % substs] + arg_list
                            ),
                        },
                        params.sorry,
                    ),
                ...
                emit_named(
                    "%s_%s_ptr_new" % (self.name, ref.name),
                    params,
                    make_proof(
                        "ptr_set_modifies_proof",
                        {
                            "fun_name": "%s_%s_ptr_new" % (self.name, ref.name),
                            "args": ", ".join(
                                ["\\<acute>ret__struct_%(name)s_C" % substs] + arg_list
                            ),
                        },
            ...
            if len(arg_list) == 0:
                ...
                for field in ref.visible_order:
                    ...
                    mask = field_mask_proof(
                        self.base, self.base_bits, self.base_sign_extend, high, size
                    )
                    sign_extend = sign_extend_proof(
                        high, self.base_bits, self.base_sign_extend
                    )
            ...
            if self.sliced_tag:
                ...
                if any([w for w in self.widths if w < size]):
                    tag_mask_helpers = "%s_%s_tag_mask_helpers" % (self.name, ref.name)
                else:
                    tag_mask_helpers = ""
            ...
            for field, _, size, high in ref.fields:
                ...
                if not params.skip_modifies:
                    emit_named(
                        "%s_%s_get_%s" % (self.name, ref.name, field),
                        params,
                        make_proof(
                            "const_modifies_proof",
                            {
                    ...
                    emit_named(
                        "%s_%s_ptr_get_%s" % (self.name, ref.name, field),
                        params,
                        make_proof(
                            "const_modifies_proof",
                            {
                ...
                if not params.skip_modifies:
                    emit_named(
                        "%s_%s_set_%s" % (self.name, ref.name, field),
                        params,
                        make_proof(
                            "const_modifies_proof",
                            {
    ...
    def generate_hol_defs(self, params):
        ...
        for name, value, ref in self.tags:
            if ref in empty_blocks:
                ...
                constructor_exprs.append(
                    "%s %s_CL" % (gen_name(name, True), gen_name(name))
                )
        ...
        subs = {"name": self.name, "base": self.base}
        ...
        if self.sliced_tag:
            fs = (
                union_get_tag_def_header_template % subs
        ...
        if self.sliced_tag:
            fs = (
                union_get_tag_eq_x_def_header_template % subs
                + self.compressed_tag_expr(
        ...
        if not self.sliced_tag:
            for name, value, ref in self.tags:
                ...
                if part_widths:
                    ...
                    fs = (
                        union_tag_mask_helpers_header_template % subs
                        + "".join(
                            [
                                union_tag_mask_helpers_entry_template
                                % dict(
                                    subs,
                                    part_mask=hex(2**pw - 1),
        ...
        for name, value, ref in self.tags:
            ...
            tag_cases.append(
                "if tag = scast %s then Some (%s)" % (gen_name(name), value)
            )

            collapse_proofs += make_proof(
                "lift_collapse_proof",
                {"name": self.name, "block": name, "value": value},
                params.sorry,
            )
            collapse_proofs += "\n\n"
    ...
    def compressed_tag_expr(self, source, code=True):
        def mask(size):
            return (
                f"0x{(1 << size) - 1:x}{self.constant_suffix}"
                if code
                else f"mask {size}"
            )
    ...
    def generate(self, params):
        ...
        if len(self.tags) > 0:
            ...
            print("    %s_%s = %d" % (self.name, name, value), file=output)
        print("};", file=output)
        print("typedef enum %s_tag %s_tag_t;" % (self.name, self.name), file=output)
        print(file=output)
        ...
        if self.sliced_tag:
            fs = (
                tag_reader_header_template % subs
                + "    return "
        ...
        if self.sliced_tag:
            fs = (
                tag_eq_reader_header_template % subs
                + "    return ("
                + self.compressed_tag_expr(f"{self.name}.words[{self.tag_index}]")
                + ") == ex_type_tag;"
        ...
        if self.sliced_tag:
            fs = (
                ptr_tag_reader_header_template % subs
                + "    return "
        ...
        for name, value, ref in self.tags:
            # Generate generators
            param_fields = [
                field for field in ref.visible_order if field not in self.tag_slices
            ]
            param_list = [
                "%s %s" % (TYPES[options.environment][self.base], field)
            ...
            ptr_params = ", ".join(
                ["%s_t *%s_ptr" % (self.name, self.name)] + param_list
            )
            ...
            for field in ref.visible_order + (
                [self.tagname] if self.sliced_tag else []
            ):
                ...
                if field == self.tagname and self.sliced_tag:
                    ...
                    f_value = "(%s)%s_%s" % (
                        TYPES[options.environment][self.base],
                        self.name,
                        name,
                    )
                    offset, size, high = ref.field_map[field]
                ...
                if high:
                    ...
                    if self.base_sign_extend:
                        high_bits = (
                            (self.base_sign_extend << (self.base - self.base_bits)) - 1
                        ) << self.base_bits
                    else:
                        high_bits = 0
                    if shift < 0:
            ...
            def mk_inits(prefix):
                return "\n".join(
                    ["    %s%s" % (prefix, word_init) for word_init in word_inits]
                )
            ...
            for field, offset, size, high in ref.fields:
                ...
                emit_named(
                    "%s_%s_ptr_get_%s" % (self.name, ref.name, field),
                    params,
                    ptr_union_reader_template % subs,
                )
                ...
                emit_named(
                    "%s_%s_ptr_set_%s" % (self.name, ref.name, field),
                    params,
                    ptr_union_writer_template % subs,
                )

    def make_names(self):
        "Return the set of candidate function names for a union"

        substs = {"union": self.name, "tagname": self.tagname}
        names = [
            t % substs
            for t in [
    ...
    def represent_value(self, value, width):
        ...
        tail_str = "{:0{}b}".format(value, width) + "_" * (
            self.tag_offset[width] - self.class_offset
        )
        head_str = "_" * (
            (max_width + self.tag_offset[max_width] - self.class_offset) - len(tail_str)
        )
    ...
    def represent_class(self, width):
        ...
        return "{:0{}b}".format(cmask, max_width).replace("0", "-") + " ({:#x})".format(
            cmask
        )
    ...
    def represent_field(self, width):
        ...
        return (
            "{:0{}b}".format((2**width - 1) << offset, max_width)
            .replace("0", "-")
            .replace("1", "#")
        )
    ...
    def assert_value_in_class(self, name, value, width):
        ...
        for w, mask in [(lw, self.classes[lw]) for lw in self.widths if lw < width]:
            if (cvalue & mask) != mask:
                raise ValueError(
                    "The value for element %s of tagged union %s,\n"
                    ...
                    % (
                        ...
                        width,
                        w,
                        self.represent_class(w),
                    )
                )

        if (
            self.widths.index(width) + 1 < len(self.widths)
            and (cvalue & self.classes[width]) == self.classes[width]
        ):
            raise ValueError(
                "The value for element %s of tagged union %s,\n"
    ...
    def make_classes(self, params):
        ...
        for uw in used_widths:
            if uw not in classes:
                raise ValueError(
                    "%s: none defined for a field of %d bits." % (context, uw)
                )
        ...
        for w in widths:
            ...
            if (classes[w] << class_offset) | offset_field != offset_field:
                raise ValueError(
                    ...
                    "exceeds the field bounds:\n  {:s}.".format(
                        context, w, self.represent_class(w), self.represent_field(w)
                    )
                )

        if len(widths) > 1 and classes[widths[0]] == 0:
            raise ValueError("%s: the first (width %d) is zero." % (context, widths[0]))

        if any(
            [
                classes[widths[i - 1]] == classes[widths[i]]
        ...
        if params.showclasses:
            ...
            for w in widths:
                print(
                    "{:2d} = {:s}".format(w, self.represent_class(w)), file=sys.stderr
                )
...
class Block:
    def __init__(self, name, fields, visible_order):
        ...
        self.fields = _fields
        self.field_map = dict(
            (name, (offset, size, high)) for name, offset, size, high in _fields
        )
        ...
        if not visible_order is None:
            ...
            for _name in visible_order:
                if _name not in self.field_map:
                    raise ValueError("Nonexistent field '%s' in visible_order" % _name)
                missed_fields.remove(_name)

            if len(missed_fields) > 0:
                raise ValueError(
                    "Fields %s missing from visible_order"
    ...
    def set_base(self, base, base_bits, base_sign_extend, suffix):
        ...
        self.base_sign_extend = base_sign_extend
        if self.size % base != 0:
            raise ValueError("Size of block %s not a multiple of base" % self.name)
        self.multiple = self.size // base
        for name, offset, size, high in self.fields:
            if offset // base != (offset + size - 1) // base:
                raise ValueError(
                    "Field %s of block %s "
                    "crosses a word boundary" % (name, self.name)
                )

    def generate_hol_defs(
        self, params, suppressed_fields=[], prefix="", in_union=False
    ):
        output = params.output
    ...
    def generate_hol_proofs(self, params, type_map):
            ...
            emit_named(
                ...
                make_proof(
                    "const_modifies_proof",
                    {
                        ...
                        "args": ", ".join(
                            ["\\<acute>ret__struct_%s_C" % self.name] + arg_list
                        ),
                    },
                    params.sorry,
                ),
        ...
        for field in self.visible_order:
            ...
            field_eq_list.append(
                "%s_CL.%s_CL = %s(\\<^bsup>s\\<^esup>%s AND %s)"
        ...
        emit_named_ptr_proof(
            ...
            params.toplevel_types,
            "ptr_new_spec",
            {"name": self.name, "args": ", ".join(arg_list), "field_eqs": field_eqs},
        )
                ...
                emit_named(
                    ...
                    make_proof(
                        "ptr_set_modifies_proof",
                        {
                            ...
                            "args": ", ".join(
                                ["\\<acute>%s_ptr" % self.name, "\\<acute>v%(base)d"]
                            ),
                        },
                        params.sorry,
                    ),
    ...
    def generate(self, params):
        ...
        for field, offset, size, high in self.fields:
            ...
            # Reader
            emit_named("%s_get_%s" % (self.name, field), params, reader_template % subs)

            # Writer
            emit_named("%s_set_%s" % (self.name, field), params, writer_template % subs)

            # Pointer lifted reader
            emit_named(
                "%s_ptr_get_%s" % (self.name, field), params, ptr_reader_template % subs
            )

            # Pointer lifted writer
            emit_named(
                "%s_ptr_set_%s" % (self.name, field), params, ptr_writer_template % subs
            )
    ...
    def make_names(self, union=None):
        ...
        if union is None:
            ...
            names = [t % substs for t in ["%(block)s_new", "%(block)s_ptr_new"]]
        else:
            substs = {"block": self.name, "union": union.name}
            ...
            names = [
                t % substs
                for t in ["%(union)s_%(block)s_new", "%(union)s_%(block)s_ptr_new"]
            ]
...
class OutputFile(object):
    def __init__(self, filename, mode="w", atomic=True):
        ...
        if atomic:
            ...
            if DEBUG:
                print(
                    "Temp file: %r -> %r" % (self.file.name, self.filename),
                    file=sys.stderr,
                )
...
if __name__ == "__main__":
    ...
    if options.hol_defs:
        ...
        print(
            'imports "%s/KernelState_C"'
            % (os.path.relpath(options.cspec_dir, os.path.dirname(out_file.filename))),
            file=out_file,
        )
        ...
        def is_bit_type(tp):
            return umm.is_base(tp) & (
                umm.base_name(tp)
                in map(lambda e: e.name + "_C", det_values(blocks, unions))
            )
        ...
        print(
            "\n".join(
                map(lambda x: "#include <%s>" % x, INCLUDES[options.environment])
            ),
            file=out_file,
        )
        for e in det_values(blocks, unions):
